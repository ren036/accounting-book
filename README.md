...
- 统计页按年份、月份查看收支汇总。
- 查看某个月的账单明细。
- 管理通用储蓄和自定义专项资金，支持分次存入、取出及目标进度。
- 根据初始余额、收支和储蓄流水计算当前可支配余额，历月余额自动结转。
- 可直接按实际金额校准当前可支配余额，自动抵消导入历史账单造成的余额偏差。
- 专项资金可编辑名称、目标金额和目标日期，状态分为存钱中、已使用和已取消；达到目标金额后自动显示为已存够。
- 专项资金独立于账单：存入、取出只影响储蓄总额和当前可支配金额，不进入收支、日常消费或预算统计。
- 专项标记为“已使用”后从当前储蓄总额中移除；标记为“已取消”时，剩余金额自动退回当前可支配。
- 查看近六个月储蓄趋势，并按目标日期计算每月建议存款。
- 可在首页“我的储蓄”卡片中一键隐藏储蓄相关金额，隐藏时统一显示为星号，并记住上次选择。
- 支出可标记为“非日常支出”：仍减少可支配金额，但不进入日常消费和预算；统计页可切换查看全部支出或日常消费。
- 设置页导出 JSON 备份。
- 设置页导入 JSON 备份。
...
储蓄数据单独保存在 `savingsBuckets` 和 `savingsMovements` 表中。存入储蓄不是支出，取出储蓄也不是收入，因此不会影响原有收支和预算统计。JSON 与 Excel 备份都会同时包含账单、储蓄目标、储蓄流水及初始可支配余额。
...
仅支持本应用导出的 Excel 备份文件。导入完成后会显示成功条数和跳过条数。
